...
- компактный стартовый экран без обязательного скролла с отдельной кнопкой `Что внутри Geo-quiz`;
- правая колонка главного экрана собрана во вкладки: `Лидерборд`, `Как играть`, `Предложить вопрос`;
- форма предложения пользовательского вопроса;
- фото объекта в карточке вопроса;
- просмотр изображения в отдельном окне по клику;
- подсказка открывается только по нажатию;
- одноразовая onboarding-инструкция только для первого раунда с возможностью закрытия;
- адаптивный zoom карты после ответа: чем ближе игрок к цели, тем точнее фокус;
- блок `Интересный факт` выделен как отдельный акцентный элемент в карточке результата;
...
- `home.png` — стартовый экран с компактной шапкой, выбранным режимом и вкладками правой панели
- `game.png` — игровой экран с картой, фото вопроса, акцентным блоком факта и одноразовой подсказкой первого раунда
